...
class Backgrounds():
    def __init__(self, background_file):
        self._images = pickle.load(open(background_file, 'rb'))
...
def camera_view_bounds_2d(scene, cam_ob, me_ob):
    ...
    try:
        ...
        write_file(output_folder+'/'+logfile,"def camera_view_bounds_2d::",msg)


def render(obj, angle_x,angle_y,angle_z, axis, index, axis_index):
    '''
    ...
    if os.path.exists(render_name):
        ...
        new_img = overlay_transparent(bckgrnd, new_img, 0, 0)
        # write the base image to disk
        img_file_name=image_directory + "/" + str(index) + "_" + axis + "_{}.png".format(axis_index)
        cv2.imwrite(img_file_name,new_img)
        upload_file(img_file_name, bucket, object_name='output/JPEGImages/'+str(index) + "_" + axis + "_{}.png".format(axis_index))
...
#            f.write(str(new_box))
        ...
        write_file(image_directory + "/" + str(index) + "_" + axis + "_{}.txt".format(axis_index),"",str(new_box))
...
def orchestrate(three_d_obj):
    ...
    try:
        ...
        for z in range(1, upper_bound):
            ...
            render(obj, 0,0,angle, "z", image_index, z)
            image_index += 1

        # Rotate on the X axis
        for x in range(1, upper_bound):
            angle = (start_angle * (math.pi/180)) + (x*-1) * (theta * (math.pi/180))
            render(obj, angle,0,0, "x", image_index, x)
            image_index += 1

        # Rotate on the Y axis
        for y in range(1, upper_bound):
            angle = (start_angle * (math.pi /180)) + (y*-1) * (theta * (math.pi/180))
            render(obj, 0,angle, 0,"y", image_index, y)
            image_index += 1

    except Exception as msg:
        #with open(output_folder+'/'+logfile, "w") as f:
        #    f.write(str(msg))
        write_file(output_folder+'/'+logfile,"",str(msg))


def main_3(three_d_object):
    global class_name
    global image_directory
    global annot_directory
...
if __name__ == '__main__':
    ...
    backgrounds = Backgrounds(background_file)
    time.sleep(3)
    main_3(three_d_object)
